...
const FlightArc: React.FC<{ route: FlightRoute; radius: number; dataVersion: number }> = ({ route, radius, dataVersion }) => {
  ...
  const { curve, lineDistances, distanceFactor, midpoint } = useMemo(() => {
  ...
  return (
    <group>
      ...
      >
        ...
        <lineBasicMaterial color="#3fa9ff" linewidth={6} transparent opacity={0.25} blending={THREE.AdditiveBlending} depthWrite={false} />
      </line>
      ...
      <line ref={coreLineRef}>
        ...
        <lineDashedMaterial color="#3fa9ff" linewidth={1} dashSize={0.04} gapSize={0.017} transparent opacity={isActive ? 0.98 : 0.7} />
      </line>
        ...
        <>
          ...
          {trailRefs.current.map((ref, idx) => (
            <mesh ref={ref} key={`trail-${idx}`}>
              ...
              <meshBasicMaterial color="#3fa9ff" transparent opacity={0.2} depthWrite={false} toneMapped={false} />
            </mesh>
          ))}
        </>
      )}
