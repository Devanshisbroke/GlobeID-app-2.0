...
class TripWeatherCard extends StatelessWidget {
  ...
  List<_DayData> _seasonalData(String dest, int month) {
    ...
    final seasonal = (month >= 6 && month <= 8)
        ? 8
        : (month >= 12 || month <= 2)
            ? -5
            : 3;
...
class _DayData {
  const _DayData(
      {required this.label, required this.temp, required this.icon, this.rain});
  final String label;
  final int temp;
  final IconData icon;
  final String? rain;
...
class _DayCol extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Icon(day.icon,
          size: 20,
          color: day.icon == Icons.wb_sunny_rounded
      ...
      if (day.rain != null)
        Text(day.rain!,
            style: theme.textTheme.labelSmall
                ?.copyWith(fontSize: 9, color: const Color(0xFF0EA5E9))),
    ]);
  }
}
...
class TripTimezoneCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final jetLag = offsetHours.abs() > 5
        ? 'Severe'
        : offsetHours.abs() > 2
            ? 'Moderate'
            : 'Minimal';
    final dir = offsetHours > 0
        ? 'East'
        : offsetHours < 0
            ? 'West'
            : 'Same';
      ...
      child: Column(children: [
        Row(children: [
          ...
          Column(children: [
            ...
            Text('${offsetHours >= 0 ? '+' : ''}${offsetHours}h',
                style: theme.textTheme.labelSmall?.copyWith(
                    fontWeight: FontWeight.w800,
          ...
          Expanded(
              child: _TimeCol(
                  city: destination,
                  time:
                      '${destTime.hour.toString().padLeft(2, '0')}:${destTime.minute.toString().padLeft(2, '0')}',
                  label: 'DESTINATION',
                  tone: const Color(0xFFEC4899))),
        ]),
        const SizedBox(height: AppTokens.space2),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Text('Jet lag: $jetLag · Direction: $dir',
              style: theme.textTheme.labelSmall?.copyWith(
...
class TripVisaCard extends StatelessWidget {
  const TripVisaCard(
      {super.key, required this.citizenship, required this.destination});
  ...
  _VisaReq _lookup(String from, String to) {
    ...
    return switch (h) {
      0 => _VisaReq('Visa Free', '90-day stay, valid passport required', 'FREE',
          const Color(0xFF22C55E), Icons.check_circle_rounded),
      1 => _VisaReq('e-Visa', 'Apply online 72h before departure', 'E-VISA',
          const Color(0xFF0EA5E9), Icons.language_rounded),
      2 => _VisaReq('Visa on Arrival', 'Available at airport, \$25 fee', 'VOA',
          const Color(0xFFF59E0B), Icons.flight_land_rounded),
      _ => _VisaReq('Visa Required', 'Consular application needed, 10+ days',
          'REQUIRED', const Color(0xFFEF4444), Icons.warning_rounded),
    };
  }
}

class _VisaReq {
  const _VisaReq(this.status, this.detail, this.badge, this.color, this.icon);
  final String status, detail, badge;
  final Color color;
  final IconData icon;
}
